...
const ShortenItem = ({ originalUrl, shortenUrl, clickCountl, createdDate }) => {
    ...
    const analyticsHandler = (shortenUrl) => {
        if (!analyticToggle) {
            setSelectedUrl(shortenUrl);
        }
        setAnalyticToggle(!analyticToggle);
    }

    const fetchMyshortenUrl = async () => {
        setLoader(true);
        try {
    ...
    <div className={`flex sm:flex-row flex-col  sm:justify-between w-full sm:gap-0 gap-5 py-5 `}>
      <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
        <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2 ">
            {/* <a href={`${import.meta.env.VITE_REACT_SUBDOMAIN}/${shortenUrl}`}
                target="_blank"
                className=" text-[17px]  font-montserrat font-[600] text-linkColor ">
                {subDomain + "/" + `${shortenUrl}`}
            </a> */}

            <Link
              target='_'
              className='text-[17px]  font-montserrat font-[600] text-linkColor'
              to={import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + `${shortenUrl}`}>
                  {subDomain + "/s/" + `${shortenUrl}`}
            </Link>
        ...
        <div className="flex items-center gap-1 ">
          ...
          <div className="flex   items-center gap-8 pt-6 ">
            <div className="flex gap-1  items-center font-semibold  text-green-800">
              ...
              <span className="text-[16px]">{clickCountl}</span>
              <span className="text-[15px] ">
                {clickCountl === 0 || clickCountl === 1 ? "Click" : "Clicks"}
              </span>
            </div>
        ...
        <div className="flex  flex-1  sm:justify-end items-center gap-4">
            <CopyToClipboard
                ...
                text={`${import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + `${shortenUrl}`}`}
            >
            ...
            <div
                onClick={() => analyticsHandler(shortenUrl)}
